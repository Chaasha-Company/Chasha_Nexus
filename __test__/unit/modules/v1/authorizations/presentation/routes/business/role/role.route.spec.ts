...
import { businessRoleRouter } from '@/modules/v1/authorizations/presentation/routes/business/role/role.route';
...
describe('businessRoleRouter', () => {
  const routes = routesOf(businessRoleRouter);
  ...
  it('ends every mutation chain with its dedicated controller after guard and validation middlewares', () => {
    const expectations: [string, string, string][] = [
      ['post', '/create', 'createBusinessRoleController'],
      ['patch', '/update', 'updateBusinessRoleController'],
      ['post', '/delete', 'deleteBusinessRoleController'],
  ...
  it('ends every read chain with its dedicated controller', () => {
    const expectations: [string, string, string][] = [
      ['get', '/get-all', 'getAllBusinessRoleController'],
      ['get', '/list-option', 'getListOptionBusinessRoleController'],
      ['get', '/get-all-permissions', 'getBusinessRolePermissionsController'],
    ];
