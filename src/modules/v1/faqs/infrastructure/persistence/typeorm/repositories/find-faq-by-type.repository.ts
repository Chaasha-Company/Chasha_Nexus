...
export const findAllFaqByTypeRepository =
  ...
  async (faqData: FindAllFaqByTypeQuery, ctx?: TransactionContext): Promise<PaginationResponseRepository<FaqsModel>> => {
    ...
    const cacheKey = ['faqs', faqData.faqTypeId, faqData.faqSearchQuery ?? '', faqData.faqPaginationSkip, faqData.faqPaginationTake].join(':');
