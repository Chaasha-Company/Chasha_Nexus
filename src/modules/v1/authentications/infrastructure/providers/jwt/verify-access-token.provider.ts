...
import type { VerifyAccessTokenProviderContract } from '@/modules/v1/authentications/domain';
import jwt from 'jsonwebtoken';
import { EnvValueConfig } from '@/config/env';

export const verifyAccessTokenProvider =
  (): VerifyAccessTokenProviderContract =>
  (token: string): string | BusinessEmployeeAuthTokenPayload | PlatformAdminAuthTokenPayload | null => {
    try {
      const tokenPayload = jwt.verify(token, EnvValueConfig.JWT_ACCESS_TOKEN_SECRET_KEY);
